...
def preprocess_sensor_data(timestamps_us, mpu_accel, mpu_gyro, adxl_accel, altitude):
    ...
    mpu_accel_offset = np.array([0.1,-3.4,-107], dtype=np.float32)
    #mpu_gyro_offset  = np.array([-47.59,38.8,15.8], dtype=np.float32)
    mpu_gyro_offset  = np.array([-36.4,41,15.1], dtype=np.float32)
    adxl_accel_offset = np.array([1.896,-4.62,4.0], dtype=np.float32)
...
# --- Função genérica para plot FFT ---
def plot_fft_dB(signal, fs, axis_label, tipo, ax=None, use_db=False):
    """
    Plota a FFT de um sinal removendo o componente DC.
    ...
    ------------
    signal : array
        Sinal 1D a ser analisado
    fs : float
        Frequência de amostragem [Hz]
    axis_label : str
        'X', 'Y', 'Z' ou 'Altitude'
    tipo : str
        Exemplo: 'MPU Accel', 'MPU Gyro', 'ADXL Accel', 'BMP Altitude'
    ax : matplotlib.axes, opcional
        Eixo de plot para subplot
    use_db : bool
        Se True, converte magnitude para decibéis (20*log10)
    """
    N = len(signal)
    signal_zero_mean = signal - np.mean(signal)  # remove componente DC
    fft_vals = rfft(signal_zero_mean)
    freqs = rfftfreq(N, 1/fs)
    ...
    # Converte para escala logarítmica se necessário
    if use_db:
...
def fft_accel_mpu_dB(time_s, mpu_accel, use_db=False):
    fs = 1 / np.mean(np.diff(time_s))
    fig, ax = plt.subplots(3,1, figsize=(8,6), sharex=True)
    for i, axis in enumerate(['X','Y','Z']):
        plot_fft_dB(mpu_accel[:,i], fs, axis, "MPU Accel", ax[i], use_db)
        ax[i].set_ylabel("Magnitude" + (" [dB]" if use_db else " [m/s²]"))
    ax[-1].set_xlabel("Frequência [Hz]")
    fig.suptitle("FFT Aceleração MPU")
    plt.tight_layout()
    plt.show()

def fft_gyro_mpu_dB(time_s, mpu_gyro, use_db=False):
    fs = 1 / np.mean(np.diff(time_s))
    fig, ax = plt.subplots(3,1, figsize=(8,6), sharex=True)
    for i, axis in enumerate(['X','Y','Z']):
        plot_fft_dB(mpu_gyro[:,i], fs, axis, "MPU Gyro", ax[i], use_db)
        ax[i].set_ylabel("Magnitude" + (" [dB]" if use_db else " [°/s]"))
    ax[-1].set_xlabel("Frequência [Hz]")
    fig.suptitle("FFT Velocidade Angular MPU")
    plt.tight_layout()
    plt.show()

def fft_accel_adxl_dB(time_s, adxl_accel, use_db=False):
    fs = 1 / np.mean(np.diff(time_s))
    fig, ax = plt.subplots(3,1, figsize=(8,6), sharex=True)
    for i, axis in enumerate(['X','Y','Z']):
        plot_fft_dB(adxl_accel[:,i], fs, axis, "ADXL Accel", ax[i], use_db)
        ax[i].set_ylabel("Magnitude" + (" [dB]" if use_db else " [m/s²]"))
    ax[-1].set_xlabel("Frequência [Hz]")
    fig.suptitle("FFT Aceleração ADXL")
    plt.tight_layout()
...
def fft_altitude_bmp_dB(time_s, altitude, use_db=False):
    fs = 1 / np.mean(np.diff(time_s))
    fig, ax = plt.subplots(1,1, figsize=(8,4))
    plot_fft_dB(altitude, fs, "Altitude", "BMP Altitude", ax, use_db)
    ax.set_ylabel("Magnitude" + (" [dB]" if use_db else " [m]"))
    ax.set_xlabel("Frequência [Hz]")
    fig.suptitle("FFT Altitude BMP")
    plt.tight_layout()
    plt.show()
...
def pre_analise_frequencia(time_s, mpu_accel, mpu_gyro, adxl_accel, altitude, use_db=False):
    """
    Executa FFT de todos os sinais e plota:
    - MPU aceleração
    - MPU giroscópio
    ...
    use_db: bool
        Se True, exibe magnitude em dB.
    """
    fft_accel_mpu_dB(time_s, mpu_accel, use_db)
    fft_gyro_mpu_dB(time_s, mpu_gyro, use_db)
    fft_accel_adxl_dB(time_s, adxl_accel, use_db)
    #fft_altitude_bmp_dB(time_s, altitude, use_db)
...
#pre_analise_frequencia(time_s_interp,mpu_accel_interp, mpu_gyro_interp, adxl_accel_proc, adxl_accel_interp, use_db=True)
...
def filtrar_dados(time_s, FS, FS_novo, mpu_accel, mpu_gyro, adxl_accel, altitude_proc, ativar=True):
    ...
    # --- FIR PA 0,25Hz para giroscópio MPU ---
    fc_pa = 0.25
    ordem_fir = 101
    Wn_pa = fc_pa / nyq
    ...
    for nome, dados in sensores.items():
        fig, ax = plt.subplots(3,1, figsize=(8,6), sharex=True)
        for i, eixo in enumerate(['X','Y','Z']):
            ax[i].plot(time_s, dados[:,i], '-', label=f"{nome} Original {eixo}",lw=1,color='gray', alpha=0.5)
            ax[i].plot(time_s, sensores_filt[nome][:,i], '-', label=f"{nome} Filtrado {eixo}", lw=1.2, color=plt.cm.viridis(0.2 + i*0.3))
            ax[i].set_ylabel(nome.split()[2])
            ax[i].grid(True, linestyle='--', alpha=0.6)
...
def obter_orientacao(acc, gyro, time_s, FS, alpha=0.99, ativar_plot=True, n_media_inicial=50):
    ...
    # printar a média da aceleração inicial de cada eixo
    print(f"Aceleração média inicial (primeiras {n_inicial} amostras): ax0={ax0:.3f} g, ay0={ay0:.3f} g, az0={az0:.3f} g")
    ...
    # =============================================================
    #madgwick = Madgwick(sampleperiod=1/FS, beta=0.5)  # beta configurável
...
def diagnosticar_quaternions(quat, nome="Filtro"):
    """
    Diagnóstico de consistência dos quaternions.
    """
    print(f"\n🧩 Diagnóstico dos Quaternions — {nome}")
    print(f"Total de amostras: {len(quat)}")

    # Verifica norma de cada quaternion
    normas = np.linalg.norm(quat, axis=1)
    print(f"Norma média: {np.mean(normas):.6f}")
    print(f"Desvio padrão das normas: {np.std(normas):.6e}")
    print(f"Normas fora do intervalo [0.99, 1.01]: {np.sum((normas < 0.99) | (normas > 1.01))}")

    # Detecta NaN ou Inf
    nan_count = np.sum(np.isnan(quat))
    inf_count = np.sum(np.isinf(quat))
    if nan_count > 0 or inf_count > 0:
        print(f"⚠️ {nan_count} valores NaN e {inf_count} Inf detectados!")

    # Testa ortogonalidade de uma amostra aleatória
    from numpy.linalg import norm
    from scipy.spatial.transform import Rotation as R
    idx = np.random.randint(0, len(quat))
    Rm = R.from_quat(quat[idx, [1, 2, 3, 0]]).as_matrix()
    erro_ortogonalidade = norm(Rm @ Rm.T - np.eye(3))
    print(f"Erro de ortogonalidade (amostra {idx}): {erro_ortogonalidade:.2e}")

    # Plot rápido da norma
    import matplotlib.pyplot as plt
    plt.figure(figsize=(8,3))
    plt.plot(normas, '.', markersize=2, alpha=0.6)
    plt.axhline(1.0, color='r', linestyle='--', linewidth=1)
    plt.title(f"Norma dos Quaternions — {nome}")
    plt.xlabel("Amostra")
    plt.ylabel("Norma")
    ...
    if np.mean(normas) < 0.99 or np.mean(normas) > 1.01:
        print("⚠️ As normas estão fora do ideal. Recomenda-se normalizar:")
        print("    quat /= np.linalg.norm(quat, axis=1, keepdims=True)")
    else:
        print("✅ Quaternions normalizados corretamente.\n")

#diagnosticar_quaternions(quat_madgwick, "Madgwick")
#diagnosticar_quaternions(quat_complementar, "Complementar")
...
def corrigir_orientacao_aceleracao(acc_mpu, quat_madgwick, quat_complementar=None,
    ...
    # --- Conversão dos quaternions (SciPy usa [x, y, z, w]) ---
    rot = R.from_quat(quat[:, [1, 2, 3, 0]])

    # --- Aplica a rotação a todos os vetores de aceleração ---
    acc_global = rot.apply(acc_mpu)   # já retorna (N,3)

    # --- Remove a gravidade do eixo Z global ---
    
    acc_corrigida = acc_global - g_global
    ...
    fig, axs = plt.subplots(3, 1, figsize=(10, 6), sharex=True)
    for i in range(3):
        axs[i].plot(acc_mpu[:, i], '-', color='gray', linewidth=1 , alpha=0.6, label='Bruta (sensor)')
        axs[i].plot(acc_global[:, i], '-', color=plt.cm.viridis(0.2 + i*0.3), linewidth=1.2, label='Rotacionada (global)')
        axs[i].set_ylabel(f"Acel. {eixos[i]} [m/s²]")
        axs[i].grid(True, linestyle='--', alpha=0.6)
        axs[i].legend()
    axs[-1].set_xlabel("Amostra")
    ...
    fig, axs = plt.subplots(3, 1, figsize=(10, 6), sharex=True)
    for i in range(3):
        axs[i].plot(acc_global[:, i], '-', color=plt.cm.viridis(0.2 + i*0.3), linewidth=1.0, label='Rotacionada (global)')
        axs[i].set_ylabel(f"Acel. {eixos[i]} [m/s²]")
        axs[i].grid(True, linestyle='--', alpha=0.6)
        axs[i].legend()
    axs[-1].set_xlabel("Amostra")
    ...
    fig, axs = plt.subplots(3, 1, figsize=(10, 6), sharex=True)
    for i in range(3):
        axs[i].plot(acc_corrigida[:, i], '-', color=plt.cm.viridis(0.2 + i*0.3), linewidth=1.1, label='Corrigida (sem gravidade)')
        axs[i].set_ylabel(f"Acel. {eixos[i]} [m/s²]")
        axs[i].grid(True, linestyle='--', alpha=0.6)
        axs[i].legend()
    axs[-1].set_xlabel("Amostra")
    ...
    mag_in = np.linalg.norm(acc_mpu, axis=1)
    mag_out = np.linalg.norm(acc_global, axis=1)

    print(f"Média módulo entrada: {np.mean(mag_in):.2f} m/s²")
    print(f"Média módulo saída:   {np.mean(mag_out):.2f} m/s²")
    print(f"Razão: {np.mean(mag_out)/np.mean(mag_in):.3f}")
...
# --- Integração da aceleração com ZUPT segmentado ---

def integrar_aceleracao_com_zupt(acc_corrigida, tempo_s, zutp_mask):
    """
    Integra a aceleração corrigida em intervalos entre períodos de repouso (ZUPT),
    garantindo continuidade física da velocidade e evitando saltos abruptos.

    Parâmetros:
        acc_corrigida : np.ndarray
            Aceleração linear corrigida (Nx3)
        tempo_s : np.ndarray
            Vetor de tempo em segundos (N)
        zutp_mask : np.ndarray
            Máscara booleana (True = repouso detectado, False = movimento)

    Retorna:
        vel_masked : np.ndarray
            Velocidade integrada sem saltos (Nx3)
    """

    n = len(tempo_s)
    vel_masked = np.zeros_like(acc_corrigida)
    start = 0

    for i in range(1, n):
        # Detecção de transições repouso/movimento
        if zutp_mask[i - 1] and not zutp_mask[i]:
            # Início de movimento
            start = i

        elif not zutp_mask[i - 1] and zutp_mask[i]:
            # Fim de movimento → integra o trecho
            vel_segment = integrate.cumulative_trapezoid(
                acc_corrigida[start:i], tempo_s[start:i], axis=0, initial=0.0
            )
            vel_masked[start:i] = vel_segment
            start = i

    # Caso o movimento vá até o final do vetor
    if not zutp_mask[-1]:
        vel_segment = integrate.cumulative_trapezoid(
            acc_corrigida[start:], tempo_s[start:], axis=0, initial=0.0
        )
        vel_masked[start:] = vel_segment

    # Zera os trechos de repouso explicitamente
    vel_masked[zutp_mask] = 0.0

    return vel_masked

def integrar_movimento(acc_corrigida, tempo_s, zutp_mask, fc_fir=0.25, ordem_fir=601):
    """
    ...
    vel_masked = integrar_aceleracao_com_zupt(acc_corrigida, tempo_s, zutp_mask)
    ...
    w, h = signal.freqz(fir_coeff, worN=4096, fs=fs)
    ...
    for i in range(3):
        axs1[i].plot(tempo_s, vel[:, i], '--', color=plt.cm.viridis(0.2 + i*0.3), alpha=0.3, label=f'Original {eixos[i]}')
        axs1[i].plot(tempo_s, vel_filt[:, i], '-', color=plt.cm.viridis(0.2 + i*0.3), linewidth=1.2, label=f'Filtrada {eixos[i]}')
        axs1[i].set_ylabel(f'Vel. {eixos[i]} [m/s]')
        axs1[i].grid(True, linestyle='--', alpha=0.6)
        axs1[i].legend(loc='upper right')
        axs1[-1].set_xlabel('Tempo [s]')
    fig1.suptitle('Velocidade — Original vs Filtrada (Pós-ZUPT)', fontsize=14, weight='bold')
    plt.tight_layout()
    plt.show()
    ...
    for i in range(3):
        axs2[i].plot(tempo_s, disp[:, i], '--', color=plt.cm.viridis(0.2 + i*0.3), alpha=0.3, label=f'Original {eixos[i]}')
        axs2[i].plot(tempo_s, disp_filt[:, i], '-', color=plt.cm.viridis(0.2 + i*0.3), linewidth=1.2, label=f'Filtrado {eixos[i]}')
        axs2[i].set_ylabel(f'Desp. {eixos[i]} [m]')
        axs2[i].grid(True, linestyle='--', alpha=0.6)
        axs2[i].legend(loc='upper right')
        axs2[-1].set_xlabel('Tempo [s]')
    fig2.suptitle('Deslocamento — Original vs Filtrado (Pós-ZUPT)', fontsize=14, weight='bold')
    plt.tight_layout()
    plt.show()
    ...
    ax.plot(disp_filt[:, 0], disp_filt[:, 1], disp_filt[:, 2], 
        linewidth=2, color=plt.cm.viridis(0.1), label='Trajetória')
    ...
    # =============================================================
    # 📈 Visualização da Velocidade e Deslocamento Integrados
    # =============================================================
    
    # --- Cálculo da norma (velocidade resultante) ---
    vel_norm = np.linalg.norm(vel_filt, axis=1)   # √(vx² + vy² + vz²)
    disp_norm = np.linalg.norm(disp_filt, axis=1) # deslocamento total (opcional)
    tempo = tempo_s         # vetor de tempo (use FS da função)

    # --- Criação dos gráficos empilhados ---
    fig, axs = plt.subplots(3, 1, figsize=(10, 8), sharex=True)

    # 1️⃣ Componentes de velocidade
    axs[0].plot(tempo, vel_filt[:, 0], label='Velocidade X', color='tab:red')
    axs[0].plot(tempo, vel_filt[:, 1], label='Velocidade Y', color='tab:green')
    axs[0].plot(tempo, vel_filt[:, 2], label='Velocidade Z', color='tab:blue')
    axs[0].set_ylabel('Velocidade [m/s]')
    axs[0].set_title('Componentes da Velocidade Filtrada')
    axs[0].legend(loc='upper right', fontsize=9)
    axs[0].grid(True, linestyle='--', alpha=0.5)

    # 2️⃣ Velocidade resultante (norma)
    axs[1].plot(tempo, vel_norm, color='black', linewidth=2, label='Velocidade Resultante')
    axs[1].set_ylabel('‖v‖ [m/s]')
    axs[1].set_title('Velocidade Resultante (Norma)')
    axs[1].legend(loc='upper right', fontsize=9)
    axs[1].grid(True, linestyle='--', alpha=0.5)

    # 3️⃣ Deslocamento total (opcional)
    axs[2].plot(tempo, disp_norm, color='purple', linewidth=2, label='Deslocamento Total')
    axs[2].set_ylabel('‖d‖ [m]')
    axs[2].set_xlabel('Tempo [s]')
    axs[2].set_title('Deslocamento Total Integrado')
...
def processar_altitude_bmp(altitude_bmp, tempo_s, janela=51, polyorder=3, FS=200):
    """
    ...
    fc_pb = 6.0  # frequência de corte
    nyq_bmp = 0.5 * FS
    Wn_bmp = fc_pb / nyq_bmp
    b_bmp, a_bmp = butter(4, Wn_bmp, btype='low')

    accel_z_bmp = filtfilt(b_bmp, a_bmp, accel_z_bmp)
    vel_z_bmp = filtfilt(b_bmp, a_bmp, vel_z_bmp)
...
accel_z_bmp, vel_z_bmp, desloc_z_bmp = processar_altitude_bmp(
    ...
    janela=51, polyorder=3, FS=FS_novo
)
...
def integrar_deslocamento_bmp(accel_corr, vel_final, disp_final, accel_z_bmp, vel_z_bmp, desloc_z_bmp, time_s, mask_zutp):
    ...
    # Fundir com os dados do BMP do eixo z por um filtro complementar
    alpha_acel = 1
    alpha_vel = 0.99
    alpha_disp = 0.50
    ...
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')
    ax.plot(disp_final[:, 0], disp_final[:, 1], disp_final[:, 2], 
        linewidth=2, color='purple', label='Trajetória Filtrada')
    ax.scatter(disp_final[0, 0], disp_final[0, 1], disp_final[0, 2], 
           color='green', s=100, label='Início')
    ax.scatter(disp_final[-1, 0], disp_final[-1, 1], disp_final[-1, 2], 
           color='red', s=100, label='Fim')
    ax.set_xlabel('X [m]')
    ax.set_ylabel('Y [m]')
    ax.set_zlabel('Z [m]')
